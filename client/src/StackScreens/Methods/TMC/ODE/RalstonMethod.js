...
const RalstonMethod = ({navigation}) => {
  const handleSubmit = e => {
    e.preventDefault();
    const getData = async () => {
      try {
        await axios
          .post('http://localhost:8081/ODE/ralston', {input}) //For Flask server
          .then(res => {
            console.log(res.data);
            navigation.navigate('Ralston Method SOL', {
              data: res.data.data,
              h: input.h,
            });
          });
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>RALSTON METHOD</Text>
